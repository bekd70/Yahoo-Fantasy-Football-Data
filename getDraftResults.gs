function getDraftResults() {
  ...
  var draftResultsDataSheet = ss.getSheetByName("Draft Results");
  ...
  if (service.hasAccess()) {
    var url = `https://fantasysports.yahooapis.com/fantasy/v2/league/${GLOBALS.yearId}.l.${GLOBALS.leagueId}/draftresults`;
     try{
      var draftResponse = UrlFetchApp.fetch(url, {
    ...
    var draftXml = draftResponse.getContentText();
    var draftJson = XML_to_JSON(draftXml);
    ...
    if(draftResults!==undefined){
      ...
      while (recordCount<playerArray.length){
        ...
        try{
            ...
            var playerXml = playerResponse.getContentText();
            var playerJson = XML_to_JSON(playerXml);
